...
def _build_neighbors(holes):
    ...
    for r, c in holes:
        ...
        for di, (dr, dc) in enumerate(dirs):
            ...
            if (nr, nc) in holes:
                nb[(r,c)].append((nr, nc))
                dir_map[(r, c, nr, nc)] = di
    return nb, dir_map


def _build_dist_matrix(holes, nb):
    pl = sorted(holes)
...
NEIGHBORS, _DIRECTION_MAP = _build_neighbors(VALID_HOLES)
...
def __getattr__(name):
    if name in ("DIST_MATRIX",):
        if "DIST_MATRIX" not in _lazy:
            _lazy["DIST_MATRIX"] = _build_dist_matrix(VALID_HOLES, NEIGHBORS)
        return _lazy[name]
...
class BoardState:
    __slots__ = ("board", "current_player", "active_players",
                 "move_history", "scores", "_pieces")

    def __init__(self, active=None):
        self.board = [[0]*cfg.BOARD_COLS for _ in range(cfg.BOARD_ROWS)]
        self.current_player = 0
        self.active_players = active if active is not None else [0, 3]
        self.move_history = []
        self.scores = {p: 0 for p in range(6)}
        self._pieces = {p: set() for p in range(6)}
    ...
    def copy_from(cls, other):
        ...
        s.board = [row[:] for row in other.board]
        s.current_player = other.current_player
        s.active_players = list(other.active_players)
        s.move_history = list(other.move_history)
        s.scores = dict(other.scores)
        s._pieces = {p: set(v) for p, v in other._pieces.items()}
        return s
    ...
    def _collect_jumps(self, src, moves):
        ...
        while q:
            ...
            for nbr in NEIGHBORS[cur]:
                ...
                # 落点必须有效、为空、未被访问过
                if land not in VALID_HOLES or land in visited_land:
                    continue
    ...
    def apply_move(self, move):
        ...
        ns.board[tr][tc] = pv
        pid = pv - 1
        ns._pieces[pid].discard((fr, fc))
        ns._pieces[pid].add((tr, tc))
    ...
    def _update_score(self, pid):
        ...
        assert self.scores[pid] <= cfg.PIECES_PER_PLAYER, \
            f"player {pid} score overflow: {self.scores[pid]} > {cfg.PIECES_PER_PLAYER}"
    ...
    def undo_move(self):
        ...
        self.board[tr][tc] = 0
        self.board[fr][fc] = pv
        self._pieces[pid].discard((tr, tc))
        self._pieces[pid].add((fr, fc))
    ...
    def to_dict(self):
        ...
        return {
            ...
            "move_history": [[[fr, fc], [tr, tc], pid] for fr, fc, tr, tc, pid in self.move_history],
